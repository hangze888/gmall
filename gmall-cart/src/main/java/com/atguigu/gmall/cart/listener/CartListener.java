...
public class CartListener {
    ...
    public void listener(Long spuId){
        Resp<List<SkuInfoEntity>> listResp = pmsClient.querySkuInfoBySpuId(spuId);
        List<SkuInfoEntity> skuInfoEntities = listResp.getData();
        if(!CollectionUtils.isEmpty(skuInfoEntities)){
            skuInfoEntities.forEach(skuInfoEntity -> {
                redisTemplate.opsForValue().set(KEY_PRICE + skuInfoEntity.getSkuId(),skuInfoEntity.getPrice().toString());
            });
        }
    }
